...
//
// ### function watch (key, options)
// #### @key {string|Object} Fully qualified options, name of file store, or path.
// #### @options {string|Object} **Optional** Full qualified options, or path.
// #### @onChange {function} **Optional** onChange callback.
// Adds a new `Watch` store to this instance. Accepts the following options
//
//    nconf.watch({ file: '.jitsuconf', onChange: function onChange() {} });
//    nconf.watch({ file: '.jitsuconf' }, function onChange() {});
//    nconf.watch('path/to/config/file', function onChange() {});
//    nconf.watch('userconfig', 'path/to/config/file');
//    nconf.watch('userconfig', 'path/to/config/file', function onChange() {});
//    nconf.watch('userconfig', { file: '.jitsuconf' });
//
Provider.prototype.watch = function (key, options, onChange) {
  if (arguments.length == 1) {
    options = typeof key === 'string' ? { file: key } : key;
    key = 'watch';
  }
    ...
    if (typeof options === 'function') {
      options =  typeof key === 'string' ? { file: key, onChange: options } : key;
      key = 'watch';
    }
    ...
    if (typeof options === 'string') {
      options = { file: options, onChange: onChange };
    }
    else {
...
Provider.prototype.remove = function (name) {
  if (this.stores[name].remove) {
    this.stores[name].remove();
  }
